...
void Remove_Offered_Course(int Socket_Descriptor){
	...
	while(Bytes_Read = read(File_Descriptor, &Temporary_Record, sizeof(Temporary_Record)) > 0){
		if(Temporary_Record.Faculty_Id == NewTeach.Faculty_Id && Temporary_Record.Course_Id == NewTeach.Course_Id){
			lseek(File_Descriptor, -1 * (sizeof(Temporary_Record)), SEEK_CUR);  //Skip this record
			removed = 1;
		}
		else
			write(File_Descriptor, &Temporary_Record, sizeof(Temporary_Record));
	}
